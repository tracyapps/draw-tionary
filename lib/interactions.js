...
/**
 * Hide the answer behind Discord's click-to-reveal blur.
 *
 * Announcing a solve is the good part — somebody got it, they scored, the
 * drawer's work landed. Printing the word in the same breath ends the round
 * for every person still looking at it, and in an async game that is most of
 * the channel, hours later.
 *
 * Anyone who wants the answer is one tap away; anyone who wants to keep
 * playing scrolls past. Discord announces these as spoilers to screen readers
 * too, so the choice survives not being able to see the blur.
 *
 * Pipes inside the word would end the spoiler early. No word in the list has
 * one, but nothing stops one being added.
 */
export const spoiler = word => `||**${String(word).replaceAll("|", "")}**||`;
...
/**
 * The channel post announcing that somebody solved a round.
 *
 * A pure function beside drawingPost, rather than a string built at the call
 * site, for one reason: this is the only message in the game that contains the
 * answer and is read by people who have not earned it. Somewhere testable is
 * the right place for that.
 */
export function solvePost(round, { userId, awarded, solverIndex = 0, solverCount = 1 }) {
  return {
    /*
     * A reply, so it sits under the drawing it belongs to rather than floating
     * loose in a busy channel. The round already knows which message it is —
     * taking it from anywhere else is a second source of the same fact.
     *
     * fail_if_not_exists keeps a deleted drawing from turning a correct answer
     * into an error; the reply just posts plainly instead.
     */
    ...(round.messageId
      ? { message_reference: { message_id: round.messageId, fail_if_not_exists: false } }
      : {}),
    /*
     * The word is not in here at all.
     *
     * A spoiler blur was the first attempt and it is still one careless tap
     * from ending the round for somebody. Nobody needs the answer in the
     * channel: solvers already know it, and the replay page shows it to anyone
     * who has earned it. What the channel wants to know is that somebody got
     * it, which is the part worth celebrating anyway.
     */
    content:
      `<@${userId}> guessed it right! **+${awarded}** points` +
      (solverIndex === 0
        ? " — first to crack it."
        : `. That makes ${count(solverCount, "person", "people")} who have got it.`),
    /*
     * Renders as a name but pings nobody. Somebody solving a word at 2am
     * should not wake the channel, and a game that buzzes phones gets muted.
     */
...
async function handleModal(i, ctx) {
  ...
  return message(
    `Correct — it's ${spoiler(round.word)}! You earned **${res.awarded}** points.${bonus}\n` +
    (res.solverIndex === 0
      ? "You were first."
