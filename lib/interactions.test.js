...
import {
  TYPE, REPLY, customId, parseCustomId, handleInteraction, guessModal, drawingPost, spoiler, solvePost
...
test("a correct guess scores, announces publicly, and records the solver", async () => {
  ...
  /*
   * Public, but not a spoiler. Everyone else in the channel is still playing —
   * asynchronously, so most of them have not even opened it yet. The solve is
   * the news; the word goes behind Discord's blur.
   */
  assert.match(
    res.data.content, /\|\|\*\*lighthouse\*\*\|\|/,
    "the answer must be wrapped in a spoiler"
  );
  assert.doesNotMatch(
    res.data.content.replaceAll("||**lighthouse**||", ""), /lighthouse/i,
    "the answer appears outside the spoiler as well, which defeats it"
  );
...
test("a spoiler cannot be broken out of by the word itself", async () => {
  // No word in the list has a pipe in it. Nothing stops one being added, and
  // "hat||" would end the blur early and print the rest in the clear.
  assert.equal(spoiler("ha|t"), "||**hat**||");
  assert.equal(spoiler("lighthouse"), "||**lighthouse**||");
});

// ---------------------------------------------------------------- solve post

test("the solve announcement never contains the answer", () => {
  const round = { id: "r1", word: "lighthouse", messageId: "m1" };
  const post = solvePost(round, { userId: "sarah", awarded: 26, solverIndex: 0 });

  assert.doesNotMatch(
    JSON.stringify(post), /lighthouse/i,
    "the channel post gave the word away to everyone still playing"
  );
  assert.match(post.content, /guessed it right/i);
  assert.match(post.content, /26/);
  assert.match(post.content, /first to crack it/i);
});

test("later solvers are told how many have got it, and are not called first", () => {
  const round = { id: "r1", word: "lighthouse", messageId: "m1" };
  const post = solvePost(round, {
    userId: "evan", awarded: 20, solverIndex: 2, solverCount: 3
  });

  assert.doesNotMatch(post.content, /first/i);
  assert.match(post.content, /3 people/);
});

test("the announcement replies to the drawing and pings nobody", () => {
  const round = { id: "r1", word: "lighthouse", messageId: "m1" };
  const post = solvePost(round, { userId: "sarah", awarded: 10 });

  assert.equal(post.message_reference.message_id, "m1");

  // A drawing deleted between the guess and the reply must not turn a correct
  // answer into an error.
  assert.equal(post.message_reference.fail_if_not_exists, false);

  // Renders as a name, wakes nobody up.
  assert.deepEqual(post.allowed_mentions, { parse: [] });
});

test("a drawing with no message id still gets announced, just not as a reply", () => {
  const post = solvePost({ id: "r1", word: "lighthouse" }, { userId: "sarah", awarded: 10 });

  assert.equal(post.message_reference, undefined);
  assert.match(post.content, /guessed it right/i);
});

test("the refreshed drawing post carries the new solver count", () => {
  // What the edit-in-place actually publishes. The count on the drawing is
  // the at-a-glance answer to "how many people have got this one".
  const base = {
    id: "r1", drawerId: "tapps", word: "lighthouse", tier: "medium", points: 20,
    status: STATUS.OPEN, guesses: [], flags: [], solvers: [], postedAt: 1, createdAt: 0
  };

  assert.match(
    drawingPost(base, {}).embeds[0].description, /Nobody has solved it yet/,
    "a fresh drawing should not claim solvers"
  );

  const solved = { ...base, solvers: ["sarah", "evan", "nora"] };
  assert.match(drawingPost(solved, {}).embeds[0].description, /3 people/);

  // And still no answer in it, which is the property that matters most.
  assert.doesNotMatch(JSON.stringify(drawingPost(solved, {})), /lighthouse/i);
});
